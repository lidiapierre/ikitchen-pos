...
export async function handler(
  ...
  if (req.method === 'GET' && new URL(req.url).pathname.endsWith('/health')) {
    return new Response(
      JSON.stringify({ ok: true, function: 'close_shift' }),
  ...
  // Verify JWT and check minimum role (owner/admin required to close shifts)
  ...
  try {
    ...
    // 3. Emit audit log entry — actor_id comes from verified JWT
